...
class _Step5GoalSettingScreenState extends State<Step5GoalSettingScreen> {
  ...
  Future<void> _bootstrap() async {
    ...
    try {
      ...
      final s = userKey.isEmpty ? null : await GoalsService.fetchFromServer(userKey);
      if (s != null) {
        // 서버에서 받은 값을 화면 표시용으로 변환
        _semester1Goal = _getDisplayValueFromActual(s.goalSem1?.toStringAsFixed(1));
        _semester2Goal = _getDisplayValueFromActual(s.goalSem2?.toStringAsFixed(1));
        // 로컬 캐시 동기화 (실제 값으로 저장)
        await GoalsService.saveLocal(
            userKey,
            s.goalSem1?.toStringAsFixed(1),
            s.goalSem2?.toStringAsFixed(1)
        );
      } else {
        // 2) 서버에 없으면 로컬 캐시 폴백
        final l = userKey.isEmpty ? null : await GoalsService.loadLocal(userKey);
        _semester1Goal = _getDisplayValueFromActual(l?.goalSem1?.toString());
        _semester2Goal = _getDisplayValueFromActual(l?.goalSem2?.toString());
      }
    } finally {
      setState(() {
        _isButtonEnabled = _semester1Goal != null && _semester2Goal != null;
  ...
  // 실제 값에서 화면 표시값으로 변환
  String? _getDisplayValueFromActual(String? actualValue) {
    if (actualValue == null) return null;
    for (var entry in gradeOptionsMap.entries) {
      if (entry.value == actualValue) {
        return entry.key;
      }
    }
    return actualValue; // 매핑되지 않은 경우 원래 값 반환
  }
  ...
  Widget build(BuildContext context) {
    return StepLayout(
      ...
      onNext: (!_loading && _isButtonEnabled)
        ...
        // 화면 표시값을 실제 전송값으로 변환
        String? actualSem1 = _semester1Goal != null ? gradeOptionsMap[_semester1Goal] : null;
        String? actualSem2 = _semester2Goal != null ? gradeOptionsMap[_semester2Goal] : null;
        ...
        try {
          if (userKey.isNotEmpty) {
            // 서버 저장 (실제 값으로)
            await GoalsService.upsertToServer(
                userKey, p(actualSem1), p(actualSem2));
            // 로컬 캐시 갱신 (실제 값으로)
            await GoalsService.saveLocal(userKey, actualSem1, actualSem2);
          }
          if (!mounted) return;
          Navigator.push(
            context,
  ...
  Widget _buildDropdown(String title, String? value, ValueChanged<String?> onChanged) => Column(
    ...
    children: [
      ...
      DropdownButtonFormField<String>(
        ...
        items: gradeOptionsMap.keys  // 화면에는 범위로 표시
            .map((displayValue) => DropdownMenuItem<String>(
            value: displayValue,
