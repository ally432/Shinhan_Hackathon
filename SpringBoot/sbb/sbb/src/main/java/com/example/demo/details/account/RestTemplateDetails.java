...
@Service
public class RestTemplateDetails {
    ...
    public String findDepositProduct(String userKey, String accountNo) {
        final String url = "https://finopenapi.ssafy.io/ssafy/api/v1/edu/demandDeposit/inquireTransactionHistoryList";
        InstIds ids = nextInstIds();

        // 6개월 전 ~ 오늘
        String endDate   = ids.date();
        String startDate = LocalDateTime.now().minusMonths(6).format(DATE_FMT);

        Map<String, Object> Header = new LinkedHashMap<>();
        Header.put("apiName", "inquireTransactionHistoryList");   // ✅ URL과 일치
        Header.put("transmissionDate", ids.date());
        Header.put("transmissionTime", ids.time());
        Header.put("institutionCode", INSTITUTION_CODE);
        Header.put("fintechAppNo", "001");
        Header.put("apiServiceCode", "inquireTransactionHistoryList"); // ✅ 일치
        Header.put("institutionTransactionUniqueNo", ids.instTxnNo());
        Header.put("apiKey", API_KEY); // TODO: 외부화
        Header.put("userKey", userKey);

        Map<String, Object> body = new LinkedHashMap<>();
        body.put("Header", Header);
        body.put("accountNo", accountNo);
        body.put("startDate", startDate); // ✅ 6개월 전
        body.put("endDate", endDate);     // ✅ 오늘
        body.put("transactionType", "A"); // 전체
